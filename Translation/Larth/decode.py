"""
This is adapted from https://github.com/google/flax/blob/main/examples/wmt/decode.py
"""
import jax
...
def brevity_penalty(alpha: float, length: int) -> jax.Array:
...
def add_beam_dim(x: jax.Array, beam_size: int) -> jax.Array:
...
def flatten_beam_dim(x: jax.Array) -> jax.Array:
...
def unflatten_beam_dim(x: jax.Array, batch_size: int, beam_size: int) -> jax.Array:
...
def flat_batch_beam_expand(x: jax.Array, beam_size: int) -> jax.Array:
    """Expands the each batch item by beam_size in batch_dimension."""
    return flatten_beam_dim(add_beam_dim(x, beam_size))


def gather_beams(nested, beam_indices: jax.Array, batch_size: int, new_beam_size: int):
...
def gather_topk_beams(
    nested, score_or_log_prob: jax.Array, batch_size: int, new_beam_size: int
):
...
def beam_init(
    batch_size: int, beam_size: int, max_decode_len: int, cache: Any
) -> BeamState:
...
def beam_search(
    ...
    max_decode_len: int,
    cache: Any,
    tokens_to_logits: Callable[[jax.Array, Any], Tuple[jax.Array, Any]],
    beam_size: int = 4,
    alpha: float = 0.6,
    eos_id: int = EOS_ID,
):
    ...
    def beam_search_loop_cond_fn(state):
        ...
        # return state.cur_index < (max_decode_len - 1)
