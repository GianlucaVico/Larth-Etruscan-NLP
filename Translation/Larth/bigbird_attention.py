"""
This file has been adapted from Ithaca: https://github.com/deepmind/ithaca/blob/main/ithaca/models/bigbird_attention.py
...
def get_block_rand_mask(
    m: int, n: int, wm: int, wn: int, r: int, last_idx: int = -1
) -> np.array:
    """
...
def create_band_mask_from_inputs(
    from_blocked_mask: np.array, to_blocked_mask: np.array
) -> jax.Array:
    """
...
def create_rand_mask_from_inputs(
    from_blocked_mask: np.array, to_blocked_mask: np.array, rand_attn: np.array
) -> jax.Array:
    """
    Create 3D attention mask from a 2D tensor mask.
...
class BigBirdAttention(nn.Module):
    ...
    def __call__(
        ...
        segmentation: jax.Array | None = None,
